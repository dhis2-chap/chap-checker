...
- **Alerts guide restructured with Slack and Generic webhook as sibling `## Transports` subsections.** Previously the Slack content was 15 lines bolted on after 150 lines of webhook envelope/examples, which left the most-common transport less documented than the more-obscure one. Slack now gets its own *Setup*, *TOML options* table mirroring the webhook one, ASCII mock of the rendered Block Kit message, *Per-channel routing* recipe (two `[alerts.slack-*]` blocks fanning to different channels), *Failure modes* table (404/429/5xx/transport), and a *Rate limits* paragraph linking Slack's docs. The webhook half keeps the JSON envelope, transition object, three worked examples, and the local-receiver recipe. Inbound anchor links (`#testing-the-wiring`, `#transition-semantics`) preserved.
- **Releasing guide rewritten to match the actual cut-a-release recipe.** The old `docs/guides/releasing.md` told operators to `pip install chap-checker --upgrade` to verify (violates the uv-only project rule) and shrugged off the changelog with "if you keep one (none yet, but tag annotations work as a starting point)" — both predated the CHANGELOG and the uv-only rule in CLAUDE.md. New version walks through the full bump → tag → push recipe with explicit steps to (a) move `[Unreleased]` into a dated `[X.Y.Z]` block + update the compare-link, (b) bump `SECURITY.md`'s "Supported versions" on minor/major, and (c) verify with `uv tool upgrade chap-checker`.
- **Development guide's repo-layout block aligned with the actual `src/chap_checker/` tree.** Removed the stale `web` subcommand mention (it's `serve` since 0.4), added the `webhook.py` alerter, added `__init__.py` entries for `alerts/` and `checks/` (registration-on-import is load-bearing — worth showing), and added the `web_ui/` subtree that ships in the wheel. Also surfaces `state_store.alert_state_lock` since it's now a documented public API.
- **README's "Built-in checks" link label retitled to "Checks"** to match the renamed mkdocs nav entry.
